...
def pack_fp_to_bin(signed_exponent, signed_mantissa, exp_width, man_width):
    ...
    sign_bit = torch.signbit(signed_mantissa).to(torch.int64)

    exponent_bias = 1 if exp_width == 1 else (2 ** (exp_width - 1)) - 1
    exponent_bit = (signed_exponent + exponent_bias).to(torch.int64)
    ...
    mantissa_bit = (mantissa_bit * 2 ** (man_width)).round().to(torch.int64)
...
def bin_2_fp(bits: int | BitArray | torch.Tensor | list, exp_width: int, mant_width: int):
    if isinstance(bits, torch.Tensor):
        shape = bits.shape
        values = [
            bin_2_fp(value, exp_width, mant_width)
            for value in bits.reshape(-1).tolist()
        ]
        return torch.tensor(values, device=bits.device).reshape(shape)
