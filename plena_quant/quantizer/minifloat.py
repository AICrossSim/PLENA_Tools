"""Non-hardware minifloat quantization functions."""
...
def _minifloat_ieee_quantize(x: Tensor, width: int, exponent_width: int, exponent_bias: int | None = None):
    ...
    shifted_mantissa = is_normal*ste_clamp(ste_round(mantissa*shift-shift), shifted_mantissa_min, shifted_mantissa_max) +\
        (~is_normal)*ste_clamp(ste_round(mantissa*shift/2), shifted_mantissa_min, shifted_mantissa_max)
...
_minifloat_ieee_quantize = _minifloat_ieee_quantize
